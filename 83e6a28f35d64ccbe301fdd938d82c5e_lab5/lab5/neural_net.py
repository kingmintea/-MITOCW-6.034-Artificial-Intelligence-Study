...
class Neuron(DifferentiableElement):
    ...
    def compute_output(self):
        ...
        inputs = self.get_inputs()
        weights = self.get_weights()
        z = sum(i.output() * w.get_value() for i, w in zip(inputs, weights))
    ...
    def compute_doutdx(self, elem):
        ...
        if elem in weights:
            idx = weights.index(elem)
            c_x = inputs[idx]
            o = self.output()
            return o * (1.0 - o) * c_x.output()
            ...
            for idx, weight in enumerate(weights):
                if self.isa_descendant_weight_of(elem, weight):
                    input_node = inputs[idx]
                    child_derivative = input_node.dOutdX(elem)
                    chain_sum += weight.get_value()
            return dsigmoid * chain_sum
...
class PerformanceElem(DifferentiableElement):
    ...
    def dOutdX(self, elem):
        ...
        o = self.my_input.output()
        d = self.my_desired_val
        return (d - o) * self.my_input.dOutdX(elem)
