...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        for classifier, alpha in self.classifiers:
            prediction = classifier.classify(obj)
            weighted_total += alpha * prediction
        if weighted_total > 0:
            return 1
        else:
            return -1
    ...
    def orange_classify(self, obj):
        ...
        for classifier, alpha in self.classifiers:
            prediction = classifier.orange_classify(obj)  # orange_classify 호출
            weighted_total += alpha * prediction

        # Orange 툴킷의 형식에 맞게 -1 대신 0을 반환합니다.
        if weighted_total > 0:
            return 1
        else:
            return 0
