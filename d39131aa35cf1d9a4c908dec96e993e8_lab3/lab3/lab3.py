...
def focused_evaluate(board):
    ...
    if board.is_game_over():
        if board.is_win() == board.get_current_player_id():
            ...
            score = -1000  #tie is treated like a loss, like basic_evaluate
    else:
...
def alpha_beta_search(board, depth,
    ...
    def alphabetascore(board, depth, alpha, beta):

        if is_terminal_fn(depth, board):
            return eval_fn(board)
        best_score = NEG_INFINITY
        for move, next_board in get_next_moves_fn(board):
            score = -alphabetascore(next_board, depth - 1, -beta, -alpha)
    ...
    for move, new_board in get_next_moves_fn(board):
        val = -1 * alphabetascore(new_board, depth - 1, -beta, -alpha)
...
def better_evaluate(board):
    if board.is_game_over():
        ...
        my_longest = board.longest_chain(my_id)
        opp_longest = board.longest_chain(opp_id)
        ...
        for move, next_board in get_all_next_moves(board):
            if next_board.is_game_over() and next_board.is_win():
                if next_board.is_win() == my_id:
                    score += 900
                elif next_board.is_win() == opp_id:
                    score -= 999
...
WHAT_I_FOUND_INTERESTING = "Alpha-beta pruning with Negamax style and writing defensive heuristics."
WHAT_I_FOUND_BORING = "Debugging."
NAME = "Kim Mintae"
EMAIL = "kingmintea@naver.com"
