...
def minimax(board, depth, eval_fn = basic_evaluate,
    ...
    for move, new_board in get_next_moves_fn(board):
        val = -1 * minimax_find_board_value(new_board, depth-1, eval_fn,
                                            get_next_moves_fn,
                                            is_terminal_fn)
        if best_val == None or val > best_val[0]: # first child node) best_val = None. then starting from second .. ) val vs best_val[0]
            best_val = (val, move, new_board) # best_val[0] = val, best_val[1] = move (column to place), best_val[2] = new_board (next state)
