...
cousin_rule = IF (AND('parent (?xp) (?x)', 'parent (?yp) (?y)', 'sibling (?xp) (?yp)'), THEN('cousin (?x) (?y)'))
grandparent_rule = IF (AND('parent (?x) (?p)', 'parent (?p) (?y)'), THEN('grandparent (?x) (?y)'))
grandchild_rule = IF (AND('parent (?p) (?x)', 'parent (?y) (?p)'), THEN('grandchild (?x) (?y)'))
...
family_rules = [ same_identity_rule, sibling_rule, brother_rule, sister_rule,
                 mother_rule, father_rule, father_rule,
                 son_rule, daughter_rule, cousin_rule,
...
HOW_MANY_HOURS_THIS_PSET_TOOK = '2 hours'
WHAT_I_FOUND_INTERESTING = 'zookeeper backchaining'
